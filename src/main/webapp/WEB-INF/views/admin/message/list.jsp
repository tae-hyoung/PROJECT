			...
			<div class="row">
				<div class="col-12">
					<div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
						<p class="mb-sm-0" style="font-size:18px; color: #495057;"><i class="ri-file-edit-line"></i><strong> 쪽지목록  </strong></p>
						<div class="page-title-right">
							<ol class="breadcrumb m-0">
								<li class="breadcrumb-item"><a href="javascript: void(0);">쪽지목록</a></li>
							</ol>
						</div>
					</div>
				</div>
...
<script>
	...
	function receiveMessageList(url, sender, page){
		$.ajax({
			...
			success: function(data){
				console.log("Receive message list data: ", data);  
				...
				$.each(data, function(index, messageVO){
                    ...
                    tr.append('<td class="receiveTitle" id="receiveTitle">' + 
                    	    (messageVO.readYn === 'N' 
				...
				console.log("Error in receiveMessageList: ", error);  // 디버깅용 로그
			}
		})
	}
	...
	function initReceivePagination(startPage, totalPages, curPage){
		...
		paginationContainer.off('click', '.page-link').on('click', '.page-link', function (event) {
			...
			if (!isNaN(pageText)) {
				...
				receiveCurPage = curPage; // 현재 페이지 업데이트
		...
		$.ajax({
			...
			success: function(data){
				...
				$.each(data, function(index, messageVO){
                    ...
                    tr.append('<td class="sendTitle" id="sendTitle"><a href="#" class="link-primary link-offset-2 text-decoration-underline link-underline-opacity-25 link-underline-opacity-100-hover" data-bs-toggle="modal" data-bs-target="#Modal">' + messageVO.title + '</a></td>');
                    tr.append('<td class="content">' + messageVO.content + '</td>');
                    tr.append('<td class="sendDt" style="text-align: center;">' + regSendDt + '</td>');
	...
	$(document).on('click', '.receiveTitle', function(event) {
		...
		$.ajax({
			...
			success: function() {
				console.log("페이지 : ",receiveCurPage)
				getReceiveMessage();
				receiveMessageList(receiveUrl, sender, receiveCurPage);
			},
			error: function(error) {
