...
//실시간 사용 현황(div 회색으로 변하게 하는 & 실시간 사용자 카운트)
function liveRe() {
...
// 시작시간 시간이 현재시간보다 크면 사용완료로 상태 바꾸기
...
$(document).ready(function () {
    ...
    // 선택한 위치가  변경될 때마다 호출될 함수
    function updateTimetable() {
        let facCode = $("#inputFC").val();
        console.log("Selected Seat:", selectedSeat);

        // 위치 선택 안 하면 시간 선택 못하게 막기
        if (selectedSeat === "") {
    ...
    // 예약 버튼 클릭 이벤트 처리(리콰이어드 속성 아작스에서 처리할 려면 버튼 서브밋으로 바꾸고 이렇게 막아서 처리해주기)
  	$("#reserveFrm").submit(function(e) {
        ...
        var reservationMessage = "예약일 : " + data.regDt + "<br>시작 시간 : " + data.beginTm + 
						        "<br>종료 시간 : " + data.endTm + "<br>선택한 자리 : " + data.seat + "번 좌석" +
            ...
            success: function (result) {
                ...
                result.forEach(reserveVO => {
                    ...
                    if (beginDate === date) {
                        ...
                        row += "<td style='vertical-align: middle;'>" + reserveVO.seat + " 좌석</td>";
	...
	<div class="row">
		<div class="col-12">
			<div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
				<p class="mb-sm-0" style="font-size:18px; color: #495057;"><img alt="" src="/resources/images/독.png" style="width: 20px;">&nbsp;<strong>독서실</strong></p>
				<div class="page-title-right">
					<ol class="breadcrumb m-0">
						<li class="breadcrumb-item"><a href="javascript: void(0);">공동시설물</a></li>
						<li class="breadcrumb-item active">독서실</li>
					</ol>
				</div>
			</div>
		</div>
					...
					<c:forEach var="reserveVO" items="${reserveVOList}" varStatus="stat">
						...
						<c:if test="${fn:substring(reserveVO.beginTm, 0, 7) eq '2024/07'}">
							<tr>
								...
								<td style="vertical-align: middle;">${reserveVO.seat}번 좌석</td>
			...
			<div class="modal-content" style="width: 749px; height: 740px;">
				<div class="modal-header" style="border-bottom: 1px solid gray;">
					<h5 class="modal-title" id="timetableModalLabel"
						style="margin-bottom: 10px;">예약</h5>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close"
						style="background-color: white; border: 0px solid; font-size: 33px; margin-top: -15px; color: #979797;">
				...
				<div class="modal-body">
					...
					<div style="float: left; width: 84%; margin: 0px -10px -4px 13px;">
						<p>
							위&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;치&nbsp;&nbsp;&nbsp;<select
								name="seat" id="seat" required="required">
								<option value="">위치 선택</option>
								<c:forEach var="i" begin="1" end="20">
									<option value="${i}" id="seat${i}">${i}번좌석</option>
								</c:forEach>
							</select> <span class="text-danger" style="font-size: 14px;">&nbsp;&nbsp;※위치
								선택을 먼저 해주세요※</span>
						</p>
						<p>
							<span style="letter-spacing: 4px">인원수</span>&nbsp;<input
								type="number" name="nop" id="nop" style="width: 130px;" value="1" readonly="readonly" />&nbsp;명
						</p>
					</div>
					<div style="margin-top: 33px;">
						<div id="reAbl" style="float: left;"></div>
						<p>&nbsp; : 예약가능&nbsp;</p>
						<div id="reunAbl" style="float: left;"></div>
						<p>&nbsp; : 예약불가</p>
					</div>
